...
def quantize (W):
    ...
    Wb = tf.clip_by_value(Wb,-4*fff,4*fff)
...
def operations (Wb_pre, Wb, We):
    fff = 0.0625 * 0.125
    We = We + tf.cast(tf.abs(Wb_pre-Wb)>fff,dtype=tf.float32)
    return We
...
Wa = tf.Variable(initial_value=0*tf.zeros([784, 10]))
We = tf.Variable(initial_value=0*tf.zeros([784, 10]))
...
# Start training
Wb_pre=tf.placeholder(dtype=tf.float32,shape=[784,10])
Wb_pre_feed=tf.Variable(dtype=tf.float32,initial_value=tf.zeros([784,10])*0)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            Wee = sess.run(operations(Wb_pre, Wb, We),feed_dict={Wb_pre:Wb_pre_feed,})
            NOO = tf.reduce_sum(Wee)
            ...
            # Wb_pre_feed=sess.run(Wb)
            aa=Wb_pre_feed.assign(Wb)
            sess.run(aa)
        ...
        if (epoch + 1) % display_step == 0:
            ...
            print("Epoch:", '%04d' % (epoch + 1), "cost=", "{:.9f}".format(avg_cost),"operations:", sess.run(NOO), "N_Wb:", sess.run(N_Wb), "N_Wa:", sess.run(N_Wa))
