...
def quantize (W):
    ...
    # Wb = Wb + tf.cast(fff*5.0 < W, tf.float32) * (-W +5.0* fff)
    # gradual 부분의 에너지 소모 처리 방법 계산 필요
    ...
    Wb = tf.clip_by_value(Wb,-8*fff,8*fff)
...
We = np.zeros([784,10])
full_precision = np.zeros(1)
# Wee = tf.Variable(tf.zeros([784, 10]))
...
W_pre=tf.Variable(initial_value=tf.zeros([784,10]))
W_pre_place=tf.placeholder(dtype=tf.float32,shape=[784,10])
W_pre_assign=W_pre.assign(W_pre_place)

# N_Wb = tf.reduce_sum(Wb)
# N_Wb_pre = tf.reduce_sum(Wb_pre)
operation_count=operations(Wb_pre, Wb)
# FP_operation_count = FP_operation(W_pre, W)

# N_sub = (tf.abs(Wb-Wb_pre))
...
pred = tf.matmul(x_data, Wb) + b  # Softmax
...
cost=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_data, logits=pred))
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            W_value = sess.run(W)
            sess.run(W_pre_assign, feed_dict={W_pre_place: W_value})
            ...
            W_pre_value = W_pre.eval()
            W_value = sess.run(W)
            FP_operation_count = FP_operation(W_pre_value, W_value)
            full_precision = full_precision + FP_operation_count
            # count =sess.run(N_sub)
            # Wee = Wee + count
        ...
        #     #             print(sess.run(W))
        #
        #     # print("Epoch:", '%04d' % (epoch + 1), "cost=", "{:.9f}".format(avg_cost),"operations:", np.sum(np.sum(We)), "N_Wb:", sess.run(Wb), "N_sub:", sess.run()
        ...
        print("Epoch:", '%04d' % (epoch + 1), "cost=", "{:.9f}".format(avg_cost), "operations:", np.sum(np.sum(We)), "Full:", full_precision)
